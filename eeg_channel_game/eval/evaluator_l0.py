...
class L0Evaluator(EvaluatorBase):
    def __init__(
        self,
        *,
    ...
    def evaluate(self, key: int, fold: FoldData) -> tuple[float, dict[str, Any]]:
        ...
        # Fisher (ANOVA F) can be heavy-tailed; log1p compresses scale and makes rewards
        # comparable across subjects/folds (especially when using delta_full22 normalization).
        ch_score = np.log1p(np.maximum(stats.fisher.mean(axis=1), 0.0)).astype(np.float32, copy=False)  # [22]
